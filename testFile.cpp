...
const float WINDOW_MAX = 1000;
const float WINDOW_MIN = 0;
const float VIEWPORT_MAX =900;
...
void midpointCircle(double xCord, double yCord, int radius)
{
	xCord = (int) xCord;
	yCord = (int) yCord;
	int x, y;
	...
	while(x < y)
	{
		...
		pointSymmetry((double) xCord, (double) yCord, (double) x, (double) y);
	}
...
void display(void)
{
	...
	glColor3f(1.0, 1.0, 1.0);
	glRectf(VIEWPORT_MIN, VIEWPORT_MIN, VIEWPORT_MAX, VIEWPORT_MAX); 
...
{
	glutInit(&argc, argv);
	glutInitDisplayMode(GLUT_SINGLE|GLUR_RGB); //change to double later
